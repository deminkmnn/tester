...
static bool mx_is_visited(t_link *visited, t_island *island) {
...
static void mx_check_is_shortest(t_path *path) {
...
static void mx_find_path(t_main *main, t_link *visited, int weight, t_path **paths) {
    t_link *current_link = main->algorithm->start->links; 
    t_link *visited_island = NULL; 
    t_path *temp_path = NULL; 
    ...
    if (main->algorithm->start == main->algorithm->end) {
        temp_path = mx_graph_path_create(visited);
        mx_graph_path_push_back(&paths[main->algorithm->end->index], temp_path);
    } 
    else {
        while (current_link) {
...
static void mx_set_distances(t_path *path) {
    t_path *current_path = path;
