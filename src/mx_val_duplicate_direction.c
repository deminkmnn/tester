...
static t_pair *create_pair(const char *island1, const char *island2, int distance) {
...
static void free_pair(t_pair *pair) {
    free(pair->island1);
    free(pair->island2);
    free(pair);
}

static bool check_and_add_pair(t_pair **head, const char *island1, const char *island2, int distance) {
    t_pair *current = *head;
    while (current) {
        if ((mx_strcmp(current->island1, island1) == 0 && mx_strcmp(current->island2, island2) == 0) ||
            ...
            if (current->distance != 0) {
                return false; // Duplicate found with non-zero distance
            }
        }
